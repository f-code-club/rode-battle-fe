...
export default function ContestSummaryCard({ form, onConfirmedSubmit }: ContestSummaryCardProps) {
  const {
    control,
    trigger,
  ...
  return (
    <>
      ...
      >
        <p>
          Are you sure you want to create <strong>{contestName?.trim() || 'this contest'}</strong> with{' '}
          <strong>{problems.length}</strong> {problems.length === 1 ? 'problem' : 'problems'}?
        </p>
      </ConfirmDialog>
    </>
  );
